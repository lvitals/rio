-- ETL: Embedded Template Lua
--
-- ETL is the default template engine for the Rio framework, heavily inspired by
-- Ruby's ERB. It allows you to embed Lua code directly into your HTML (or any text)
-- files using special tags.
--
-- Syntax:
--   <% lua_code %>     : Executes Lua code without outputting anything.
--   <%= lua_expression %>: Evaluates a Lua expression, HTML-escapes the result, and outputs it.
--   <%- lua_expression %>: Evaluates a Lua expression and outputs the raw result (no escaping).
--   <%-- comment --%>  : Template comment (will not be executed or rendered).
--
-- How it works:
--   The engine compiles the template into a standard Lua function that, when executed,
--   returns the rendered string. It supports layouts, partials (via the render helper),
--   and has built-in protection against common XSS attacks by escaping HTML entities by default.
...
local setfenv, assert, type, error, tostring, tonumber, setmetatable
do
  local _obj_0 = _G
  setfenv, assert, type, error, tostring, tonumber, setmetatable = _obj_0.setfenv, _obj_0.assert, _obj_0.type, _obj_0.error, _obj_0.tostring, _obj_0.tonumber, _obj_0.setmetatable
end
...
do
  ...
  local _base_0 = {
    ...
    run = function(self, fn, env, buffer, i, ...)
      ...
      -- Wrapper for tostring to handle nil as empty string
      local safe_tostring = function(v)
        if v == nil then return "" end
        return tostring(v)
      end
      
      local ok, result = pcall(fn, safe_tostring, html_escape, buffer, i, ...)
      if not ok then
        return nil, result
      end
      return result
...
-- Rendering a template from a file containing data
local render_file
